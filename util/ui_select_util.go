...
type QueueSelctItem struct {
	Name      string
	QueueInfo config.Queue
}

type BaseSelctItem struct {
	Name  string
	Value interface{}
}

func QueueUISelect(label string, items []QueueSelctItem) int {
...
func StrUISelect(label string, itemStrs []string) string {
	...
	items := make([]BaseSelctItem, 0)
	for _, v := range itemStrs {
		items = append(items, BaseSelctItem{Name: v, Value: v})
	}
